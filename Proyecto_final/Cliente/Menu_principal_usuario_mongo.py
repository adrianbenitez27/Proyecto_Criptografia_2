...
def solicitar_datos_usuario():
    ...
    nombre_cliente = simpledialog.askstring("Nombre", "Ingresa tu nombre:")
    apellido_cliente = simpledialog.askstring("Apellido", "Ingresa tu apellido:")
    numero_cliente = simpledialog.askstring("Número de Cliente", "Ingresa tu número de cliente:")
...
def generar_llaves():
    ...
    if nombre_cliente and apellido_cliente and numero_cliente:
        nombre_claves = f"{nombre_cliente}_{apellido_cliente}_{numero_cliente}"
...
def cifra():
    ...
    if not (nombre_cliente and apellido_cliente and numero_cliente):
        messagebox.showerror("Error", "Debe ingresar su nombre, apellido y número de cliente antes de cifrar.")
        return

    # Cifra el archivo
    ruta_archivo_cifrado = procesos.cifrar_archivo_gcm(ruta_documento_cif, ruta_secreto_aes_cif)
    
    # Cambia el nombre del archivo cifrado para incluir nombre, apellido y número de cliente
    nombre_archivo_cifrado = f"{nombre_cliente}_{apellido_cliente}_{numero_cliente}.jpg.cifrado_gcm"
...
def descargar_pedido_terminado():
    ...
    if nombre_cliente and apellido_cliente and numero_cliente:
        nombre_archivo = f"{nombre_cliente}_{apellido_cliente}_{numero_cliente}_terminado.jpg.cifrado_gcm"
...
def firma():
    ...
    # Asegúrate de que se han ingresado el nombre, apellido y número de cliente
    if not (nombre_cliente and apellido_cliente and numero_cliente):
        messagebox.showerror("Error", "Debe ingresar su nombre, apellido y número de cliente antes de firmar.")
        return

    ruta_firma = procesos.firmar_documento(ruta_documento_firma, ruta_llave_privada_firma)

    # Cambia el nombre del documento firmado para incluir nombre, apellido y número de cliente
    nombre_archivo_firmado = f"firma_{nombre_cliente}_{apellido_cliente}_{numero_cliente}.txt"
    os.rename(ruta_firma, nombre_archivo_firmado)

    # Crear un nuevo archivo para el documento original con el nombre en formato especificado
    nombre_archivo_original_subida = f"{nombre_cliente}_{apellido_cliente}_{numero_cliente}.txt"
    shutil.copy(ruta_documento_firma, nombre_archivo_original_subida)

    # Subir la firma
    with open(nombre_archivo_firmado, "rb") as f:
        files = {'file': f}
    ...
    # Subir el documento original (renombrado solo para la subida)
    with open(nombre_archivo_original_subida, "rb") as f:
        files = {'file': f}
    ...
    os.remove(nombre_archivo_original_subida)
...
tk.Label(marco_informacion, text="1. Para generar las llaves, ingrese su nombre y apellido en minúsculas.\n2. Las fotos a seleccionar deben ser jpg\n3. El archivo de solicitud debe tener el siguiente formato: nombre_apellido todo en minusculas y en formato .txt.", justify=tk.LEFT).pack()
"""
# Recuadro para mostrar la información del cliente
marco_usuario = tk.LabelFrame(root, text="Mi informacion de Cliente", padx=5, pady=5)
